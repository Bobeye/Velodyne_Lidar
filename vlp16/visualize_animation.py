...
class vtkTimerCallback():
    ...
    def execute(self,obj,event):
        ...
        data, point_list, view_thresh = load_data(self.fs[self.timer_count])
        ...
        self.actor.RemoveAllViewProps()
        self.actor.AddActor(point_cloud.point_vtkActor)
        self.actor.AddActor(point_cloud.plane_vtkActor)
        self.actor.SetBackground(0.0, 0.0, 0.0)
        iren = obj
...
def vtk_visualize(fs, point_list, view_thresh):
    point_cloud = VtkPointCloud()
    x_thresh = view_thresh[0]
    y_thresh = view_thresh[1]
    z_thresh = view_thresh[2]
    ...
    cb = vtkTimerCallback(fs)
    cb.actor = renderer
    render_window_interactor.AddObserver('TimerEvent', cb.execute)
    timerId = render_window_interactor.CreateRepeatingTimer(1)
...
if __name__ == '__main__':
    ...
    for name in csvs:
        # print name
        # print (name.split('/')[8])
        t = (name.split('/')[1]).split('.')[0:2]
        ts += [int(t[0]+t[1])]
    ts = np.array(ts)
    ts_temp = ts.argsort()
    ts_ranks = np.empty(csv_n, int)
    ts_ranks[ts_temp] = np.arange(csv_n)
    csvs_sort = list(range(csv_n))
    for i in range(csv_n):
        csvs_sort[ts_ranks[i]] = csvs[i]
    files = csvs_sort
    # files = glob.glob(sys.argv[1] + '/*.csv')
    # for f in files:
    data, all_data, view_thresh = load_data(files[0])
    vtk_visualize(files, all_data, view_thresh)

    # main_animation(sys.argv[1])